      ...
      <div className="md:absolute h-[100%] md:h-auto  left-0 w-[100%] md:left-4 md:w-[70%] xl:w-[50%] bottom-0 md:bottom-3 bg-black/60 md:backdrop-blur-[2px] flex flex-col justify-end p-5 text-white rounded-2xl gap-y-2.5 ">
        ...
        <div className="flex text-neutral-10 items-baseline flex-wrap gap-2">
          ...
          <div className="text-sm md:text-md flex flex-nowrap whitespace-nowrap gap-x-1">
            {tvGenres.find(genre => genre.id === item?.genres?.[0])?.name || ""}<span className="text-primary-50"> | </span>{tvGenres.find(genre => genre.id === item?.genres?.[1])?.name || ""}<span className="text-primary-50"> | </span>{tvGenres.find(genre => genre.id === item?.genres?.[2])?.name || ""}
          </div>
        </div>
        ...
        <div className="flex space-x-4">
          <a href="#" className="flex justify-center items-center bg-primary-50 text-sm md:text-md text-black font-semibold   rounded-sm hover:bg-transparent hover:border-primary-50 hover:border hover:text-primary-50 transition duration-300  w-[100px] h-[30px] md:w-[143px] md:h-[36px]">
            WATCH
          </a>
          <a href="#" className="border group flex justify-center items-center text-sm md:text-md border-primary-50 text-primary-50 font-semibold  rounded-sm hover:bg-primary-50 hover:text-white transition duration-300  w-[100px] h-[30px] md:w-[143px] md:h-[36px]" >
            Trailer
          </a>
        </div>
      </div>
